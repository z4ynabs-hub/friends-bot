...
# ڕۆڵە ڕێگەپێدراوەکان بۆ گۆڕینی ڕەنگ
COLOR_ROLES = {
...
async def seuafraaaRyaaa(ctx):
    ...
    roles_to_add = [
        ...
        if role != ctx.guild.default_role and not role.managed and role < bot_member.top_role and role not in ctx.author.roles
    ]
    if not roles_to_add:
...
async def on_message(message):
    ...
    if content.startswith(("botcolor", "friendscolor", "xrcolor", "friendsfcolor", "_color", "trustcolor", "siscolor", "brocolor")):
        parts = content.split()
        ...
        if cmd_name in COLOR_ROLES:
            ...
            if ctx.valid:
                ...
                color_str = parts[1] if len(parts) > 1 else None
                
                role = ctx.guild.get_role(role_id)
                if role is None:
                    msg = await ctx.send("❌ ڕۆڵەکە نەدۆزرایەوە لە سێرڤەرەکەدا.", delete_after=5)
                    return
                
                if role not in ctx.author.roles:
                    msg = await ctx.send("❌ تۆ ئەم ڕۆڵەت نییە بۆ ئەوەی ڕەنگەکەی بگۆڕیت.", delete_after=5)
                    return
                
                if color_str is None:
                    msg = await ctx.send(f"❌ ڕەنگەکە بنووسە. نموونە: `{cmd_name} #000000`", delete_after=5)
                    return
                
                if not color_str.startswith("#") or len(color_str) != 7:
                    msg = await ctx.send("❌ ڕەنگەکە دەبێت بە شێوەی `#000000` بێت.", delete_after=5)
                    return
                
                try:
                    new_color = discord.Colour.from_str(color_str)
                except ValueError:
                    msg = await ctx.send("❌ ئەم Hex Color ـە دروست نییە.", delete_after=5)
                    return
                
                bot_member = ctx.guild.me
                if role >= bot_member.top_role:
                    msg = await ctx.send("❌ ڕۆڵەکە دەبێت لە خوار ڕۆڵی بۆتەکە بێت.", delete_after=5)
                    return
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    msg = await ctx.send(f"✅ ڕەنگی ڕۆڵەکە گۆڕدرا بۆ `{color_str.upper()}`.")
                    await msg.delete(delay=5)
                except discord.Forbidden:
